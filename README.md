...
- **代理与协议栈核心 (Proxy Core)**：特别感谢原作者 **Alexandre Kojève**（提供了硬核的底层 TCP/IP 协议栈手搓代码与 VLESS 穿透实现）。
- **订阅分发核心 (Sub Core)**：特别感谢原作者 **CM**（提供了稳定高效的节点聚合分发与自动化订阅转换逻辑）。
- **真实 TLS 测绘核心 (Radar Core)**：特别感谢 `Check_Proxy` 的原作者（提供了基于内嵌 TlsClient 的 100% 真实可用性验证方案）。
